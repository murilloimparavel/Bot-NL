 ...
 */
 ...
 const sendMediaVoiceNote = (client, number = null, fileName = null, delaymsg = DELAY_TIME) => {
     if(!client) return cosnole.error("El objeto cliente no está definido.");
     try { 
        setTimeout(async () => {
        number = cleanNumber(number || 0)
 ...
 * @param {*} number 
 */
const sendMessage = async (client, number = null, text = null, trigger = null, delaymsg = DELAY_TIME) => {
   setTimeout(async () => {
